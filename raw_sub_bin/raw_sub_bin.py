...
def raw_sub_bin() :
	...
	if(subsample==1):
		...
		if(color==0):
			...
			##	外层循环-高度
			##	-------------------------------------------------------------------------------------
			for i in range(0, height/skip_y):
				##	-------------------------------------------------------------------------------------
				##	内层循环-宽度
				##	-------------------------------------------------------------------------------------
				for j in range(0, width/skip_x):
				...
				if(i%2==0 || (i-1)%2==0):
					...
					for j in range(0, width/skip_x):
						if(j%2==0):
							...
							##	第i行，两个像素
							##	-------------------------------------------------------------------------------------
							output_list.append(pixel_list[i*skip_y*width+j*skip_x*2])
							output_list.append(pixel_list[i*skip_y*width+j*skip_x*2+1])
			...
			for i in range(0, height/skip_y):
				...
				if(i%2==0 || (i-1)%2==0):
					...
					for j in range(0, width/skip_x):
						...
						##	将矩阵内的数据累加
						##	-------------------------------------------------------------------------------------
						for k in range(0,skip_y*2):
							if(k%2==0):
								for l in range(0,skip_x*2):
									if(l%2==0):
										pixel	= pixel + pixel_list[i*skip_y*width+k*width+j*skip_x*2+l];
										pixel1	= pixel1 + pixel_list[i*skip_y*width+k*width+j*skip_x*2+l+1];
						##	-------------------------------------------------------------------------------------
						##	数据取平均值
						##	-------------------------------------------------------------------------------------
						pixel	= pixel/(skip_x*skip_y);
						pixel1	= pixel1/(skip_x*skip_y);
						##	-------------------------------------------------------------------------------------
						##	第i行，两个像素
						##	-------------------------------------------------------------------------------------
						output_list.append(pixel)
