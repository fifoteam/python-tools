...
##功能介绍
select_line 是一个命令行工具，可以提取出日志文件中包含某一个词语且不包含另一个词语的行。

##使用须知
select_line 会逐行分析文本，如果匹配成功，则保留下该行的信息。
分析完整个文本之后，会将解析的结果写入到 select_line.txt 文本当中。
...
### step 3
打开 命令控制台，输入如下信息
select_line f:\test\1.txt line block
line block 是关键字，一行中包含 line 且不包含 block，才能被判断出符合条件
